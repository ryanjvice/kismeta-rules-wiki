# KISMETA:

# ALCHEMISTS OF THE GREAT YEAR

### A GOODMAGIK Game

---

## Table of Contents

- [KISMETA](#kismeta)
- [COMPONENTS](#components)
  - [In the Box](#in-the-box)
  - [Anatomy of the Great Year Board](#anatomy-of-the-great-year-board)
- [GAME OVERVIEW](#game-overview)
  - [The Great Year](#the-great-year)
  - [Alignment](#alignment)
  - [Building Your Engine](#building-your-engine)
  - [The Bazaar](#the-bazaar)
  - [Transmutation Process](#transmutation-process)
- [SETUP](#setup)
  - [I. Set the Table](#i-set-the-table)
  - [II. Become an Alchemist](#ii-become-an-alchemist)
  - [III. Select a Game Mode](#iii-select-a-game-mode)
  - [IV. Build the Crucible Deck](#iv-build-the-crucible-deck)
  - [V. Determine the First Agekeeper](#v-determine-the-first-agekeeper)
  - [VI. Deal the Crucible Cards](#vi-deal-the-crucible-cards)
- [ROUND OVERVIEW](#round-overview)
  - [ROUND AT A GLANCE](#round-at-a-glance)
  - [PHASE 1: 🌸 SPRING](#phase-1--spring)
  - [PHASE 2: ☀️ SUMMER](#phase-2-️-summer)
  - [PHASE 3: 🍂 AUTUMN](#phase-3--autumn)
  - [PHASE 4: ⛰️ WINTER](#phase-4-️-winter)
- [WINNING THE GAME](#winning-the-game)
- [QUICK TIPS](#quick-tips)
- [STRATEGY GUIDE](#strategy-guide)
- [APPENDIX](#appendix)
  - [LORE: PROLOGUE ~ Alchemists Of The Great Year](#lore-prologue--alchemists-of-the-great-year)
  - [The Zodiac Wheel](#the-zodiac-wheel)
  - [Correspondence](#correspondence)
  - [KISMETA CARDS](#kismeta-cards)
  - [Stelliums](#stelliums)
  - [Card Effect Verbs](#card-effect-verbs)
  - [Crucible Cards](#crucible-cards)
  - [Tableau: Spread, Hand & Arcanum Zones](#tableau-spread-hand--arcanum-zones)
- [GLOSSARY OF TERMS](#glossary-of-terms)
- [LORE: EPILOGUE - The Veil Stirs…](#lore-epilogue---the-veil-stirs)

---

# KISMETA

Kismeta: Alchemists of the Great Year is a strategic card-and-dice game where you and up to three friends take on the roles of rival Alchemists. Your quest is to complete the Great Work, transmuting your Philosopher's Stone from Lead into Gold. Travel through the Cosmic Ages of the Great Year, pursuing this transformation under the watchful gaze of the Fates, the eternal guardians of the Crucible of Kismeta.

**Players:** 2–4

**Play Time:** 1st Play/Quickplay ~ _1–2 hrs._ | Standard ~ _2–3 hrs._ | Magnus Alchemist ~ _3+ hrs._

---

# COMPONENTS

## In the Box

**Board & Cards**

- **1 Great Year Board** (includes the Zodiac Wheel, the Mantle Ring, the Forge, Stasis, the Altar of Kismeta, the four shared Furnaces, the **Bazaar**, and a Codex panel for each player showing the Coal and Reagent costs)
- **Kismeta Card Deck** (134 cards, blue back: 112 Minor Arcana + 22 Major Arcana: 12 Adept, 10 Fate)
- **Crucible Card Deck** (22 cards, red back: Groups A, B, C, D; 4 per player, per game)

**Dice**

- **1 Cosmic Age Die** (12-sided, black; rolled by the Agekeeper each Age)
- **4 Zodiac Dice** (12-sided, 1 per player in matching player color: Red, Green, Blue, White)

**Player Pieces** (per player, in 4 colors: Red, Green, Blue, White)

- **4 Philosopher's Stones** (1 per player)
- **4 Meeples** (1 per player; marks your current Zodiac Sign on the board)
- **16 Astral Houses** (4 per player; small wooden cylinder token)

**Tokens & Resources**

- **24 Coal** (6 per player; wooden discs in each player's color; fuel for the Furnaces)
- **Reagents** (punchout tokens, one type per Element):
  🔴 Sulfur (Fire) | 🟢 Vitriol (Earth) | 🟡 Quicksilver (Air) | 🔵 Aqua Regia (Water)
- **Essence** (112 paper bills; the game's base currency) in four Elements: Fire, Earth, Air, and Water
- **1 Agekeeper's Key** (black pawn)

## Anatomy of the Great Year Board

Every ring of the Great Work is printed on the board. Working from the outside in:

- **The Zodiac Wheel.** Twelve Signs ring the board. Each Sign lists its three Aspects and what each is worth when you Align with it: +1 for its Element, +2 for its Planet, and +3 for the Sign itself. The 🏰 icon on each Sign marks where an Astral House can be built.
- **The Number Ring.** Each Sign bears a number from 1 to 12, matching the faces of the Cosmic Age Die and your Zodiac Die. When either die is rolled, it is placed on the Sign bearing that number.
- **The Mantle Ring.** Four sections, one per player, each in that player's color. Your section holds your START space, your Mantle positions 2, 4, and 6, and a Stasis space marked with a snowflake ❄️. A Stone resting on the Mantle is safe and cannot be targeted in a Siege.
- **The Forge.** Four stations at the heart of the board, passed through in order: Lead (1), Bronze (3), Silver (5), and Gold (7). There is only one of each, so every Alchemist competes for the same four stations. A Stone in the Forge is working through a Transmutation Stage, and is exposed to Siege for the single Summer it spends there.
- **The Altar of Kismeta.** The center of the board, position 8, marks the end of the Great Work. The first Stone to arrive wins the game.
- **The Furnaces.** Four rosettes at the corners of the board, one for each suit and Element: Wands and Fire (red), Cups and Water (blue), Swords and Air (yellow), and Pentacles and Earth (green). Coal is placed here to raise your Heat. Each Furnace holds three Coal in total, shared among all players and claimed first come, first served.
- **The Bazaar.** A shared reference area on the board, printed where the Stellium tracker once stood. It lists every fixed exchange rate in the game, so no one has to hunt through the rules mid-turn.
- **Codex Panels.** The four framed panels along the board's edges are your working areas. Your Codex lists the Coal and Reagent costs, and the space around it holds your Tableau: your Spread, your Hand, and your Arcanum.

---

# GAME OVERVIEW

## The Great Year

Each game of Kismeta: Alchemists of the Great Year takes you on a journey through a single Great Year, the grand cycle of the solar system spiraling around the galaxy's core. You experience each Cosmic Age, represented by a Zodiac Sign rolled at random at the start of every Age by the Agekeeper. That Sign, its ruling Planet, and its Element are the three **Aspects** of the Age, and they determine what the heavens will pay out. You roll your own Zodiac Die to set your personal Sign for the Age, which fixes where you may build an Astral House and which Furnace you may fuel.

The Cosmic Age unfolds through four Seasons, each offering distinct opportunities to advance your Great Work: Spring sets the Age and deals your cards; Summer is for crafting, trading, dueling, and sieging; Autumn harvests your Essence, fuels your Furnaces, and moves your Stone; and Winter closes the Age with wagers, card limits, and reset.

## Alignment

**Alignment** is how you measure anything you own against a Sign, and it drives both the Harvest and the Siege. Every Sign carries three Aspects: an Element, a ruling Planet, and the Sign itself. To Align something, compare its Aspects against the Sign in question and score each Aspect that matches. The values are printed on every Sign of the Zodiac Wheel.

| ASPECT MATCHED                    | VALUE |
| --------------------------------- | ----- |
| Sign                              | +3    |
| Planet                            | +2    |
| Element                           | +1    |
| All three at once (Perfect Match) | +6    |

A source that matches more than one Aspect scores every tier it matches. A source that matches all three at once scores the full stack of 6: a **Perfect Match**.

An Astral House upgraded to a **Mansion** doubles its Alignment score, whether at the Harvest or in a Siege. Other doubling effects don't stack with a Mansion's own; the higher of the two applies, not both.

💡 **Example:** the Cosmic Age is Scorpio, whose Aspects are Scorpio, Mars, and Water. A Zodiac Die sitting on Scorpio Aligns on all three and scores 6. An Astral House on Pisces shares only the Element, Water, and scores 1. A Minor Arcana card ruled by Mars shares only the Planet and scores 2.

## Building Your Engine

The Great Work runs on two core currencies: **Essence** and **Kismeta cards**.

- **Essence** is Harvested each Autumn. Your Zodiac Die, Spread, Astral Houses, and Adept Cards can each Align with the Cosmic Age. Distill 5 Essence of one Element to make a **Reagent** of that Element.
- **Kismeta cards** flow from the deck into your Tableau. Spend 3 cards of one suit to craft **Coal**. Spend 3 cards of one Planet to build an **Astral House**. Upgrade a House to an **Astral Mansion** for 2 more cards of that Planet plus a Reagent marker from the supply.

Coal and Reagents refine your currencies when a Crucible Formula demands them. Astral Houses, Adept Cards, Stelliums, and card effects tune how much you earn and how efficiently you spend. You can grow either currency across Ages; they converge when you are ready to Fire your Stone into the Forge.

Every Alchemist aims to transmute the Philosopher's Stone from Lead to Gold and place it on the Altar of Kismeta. The journey spans many Cosmic Ages, some spent building your engine, others spent driving your Stone through the Forge. In early Ages, focus on infrastructure: build Astral Houses, buy Adept Cards, hold Stelliums together in your Spread, and bank Essence.

## The Bazaar

A shared reference lives on the board, in the space the old Stellium tracker used to occupy. It lists every fixed exchange rate in the game.

| ITEM | PRICE |
| :--- | :--- |
| **Reagent** | 5 Essence of one Element |
| **Coal** | 3 cards of one suit |
| **Adept Card** | 3 Essence, of any type |
| **Rekindle a Stone** | 5 Essence, of any type |
| **Astral House** | 3 cards of one Planet |
| **Astral Mansion (upgrade)** | 2 more cards of that Planet, plus a Reagent from the supply |

Formula costs vary by Crucible Card, so check your own for Attunement, Reagents, and Heat.

## Transmutation Process

| STEP  |     | ACTION               | WHAT HAPPENS                                                                                                                                                                                          |
| ----- | --- | -------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **1** | 🎴  | **Attune**           | Assemble the Attunement printed on a Crucible Card in your Spread: a count of Element and Planet slots. Spread cards are spent to fill slots. An Astral House fills one slot per Firing and is not spent. |
| **2** | 🔥  | **Fire the Stone**   | Pay the Crucible Card's full Alchemical Formula: its Attunement, its Reagents, and the Furnace Heat it calls for. Commit up to 3 Reagents as **Shield Reagents**. Your Stone leaves the Mantle Ring and enters the Forge. |
| **3** | ⚔️  | **Defend the Stone** | Your Stone is exposed to Siege for the single Summer it spends in the Forge. Rivals must match your Shield Reagents as an Ante to attack. |
| **4** | ⏳  | **Temper the Stone** | After a full Age in the Forge, advance your Stone to the next Mantle Ring position. Set the spent Crucible Card aside and consume the Coal used for that Stage. |

_Steps 1 through 4 repeat once for each of your four Crucible Cards. Your fourth Tempering advances your Stone to the Altar of Kismeta, completing the Great Work and winning the game._

---

# SETUP

## I. Set the Table

- Place the Great Year Board in the center of the play area.
- Sort the cards into two decks: Kismeta Cards (blue back) and Crucible Cards (red back).
- **Kismeta Deck:** shuffle all cards and place the deck face down beside the board.
- **Crucible Deck:** set the Crucible cards beside the board. You build that deck in step _IV. Build the Crucible Deck._
- Set the following components within easy reach:
  - 1 Agekeeper's Key: a black pawn game piece, _(or a magical trinket from your home.)_
  - 1 Cosmic Age Die: 12-sided, black.
  - Reagent tokens and the Essence bank.

## II. Become an Alchemist

Each player chooses a color (Red, Green, Blue, or White) and collects their personal gear.

- **8 Essence:** take 2 Essence of each Element (Fire, Earth, Air, and Water).

### Components of a matching color:

- **4 Astral Houses:** wooden cylinder tokens.
- **6 Coal:** wooden discs. Keep these in your personal supply.
- **1 Meeple:** keep this nearby.
- **1 Philosopher's Stone:** place it on the START space in your section of the Mantle Ring.
- **1 Zodiac Die:** 12-sided, matching your chosen color. Each Sign on the Zodiac Wheel carries a number from 1 to 12. Whenever you roll, place the die on the Sign bearing the number you rolled.

## III. Select a Game Mode

Choose your Game Mode before continuing. The Mode you pick sets the difficulty of the Crucible Deck you build in the next step, and with it the length and weight of the game. The round structure, phases, and rules are identical in all three modes.

### 1️⃣ QUICKPLAY / FIRST PLAY

_Recommended for first playthroughs._ Weighted toward Group A and B Crucible Cards, with their smaller Attunements, lighter Reagent costs, and single-Furnace Heat requirements. Expect 1 to 2 hours.

### 2️⃣ STANDARD GAME

_Recommended once you are familiar with the rules and the flow of an Age._ A balanced spread across all four Card Groups. Expect 2 to 3 hours.

### 3️⃣ MAGNUS ALCHEMIST (MASTERY)

_For those returning from previous Great Years who seek a greater challenge._ Weighted toward Group C and D Crucible Cards, which demand mixed Planet and Element Attunements, heavy Reagent payments, and Heat in two Furnaces at once. Expect 3 hours or more.

## IV. Build the Crucible Deck

Each Crucible Card is one of the four Stage cards you work through this game to complete the Great Work. Its face shows an Alchemical Formula: an Attunement, a Reagent cost, and a Furnace Heat requirement.

- **Card Groups:** Crucible Cards are organized into four Card Groups: A, B, C, and D. Each card has a small letter in the lower left corner of its face indicating its Card Group. Groups run from A (lightest) to D (heaviest).
- Sort the Crucible Cards into Groups A through D. Shuffle each group separately.
- Choose a builder below (I. Curated Builds, or II. Random Build). Follow the table to determine how many cards to draw from each Group for your player count.
- Blindly draw the appropriate number of cards from each Group.
- Shuffle the drawn cards together to form the Crucible Deck and place it beside the board.
- Return any unused Crucible Cards to the box. They will not be used this game.

**I. CURATED BUILDS (Preset Difficulty Levels)**

| GAME MODE ➡️             | 1️⃣ QUICKPLAY BEGINNER |       |       |       | 2️⃣ STANDARD BALANCED |       |       |       | 3️⃣ MAGNUS ALCHEMIST MASTERY |       |       |       |
| ------------------------ | --------------------- | ----- | ----- | ----- | -------------------- | ----- | ----- | ----- | --------------------------- | ----- | ----- | ----- |
| **PLAYERS ⬇️**           | **A**                 | **B** | **C** | **D** | **A**                | **B** | **C** | **D** | **A**                       | **B** | **C** | **D** |
| **2 PLAYERS (8 Cards)**  | 3                     | 4     | 1     | 0     | 1                    | 4     | 2     | 1     | 0                           | 2     | 4     | 2     |
| **3 PLAYERS (12 Cards)** | 3                     | 6     | 3     | 0     | 2                    | 5     | 4     | 1     | 0                           | 3     | 6     | 3     |
| **4 PLAYERS (16 Cards)** | 4                     | 7     | 5     | 0     | 2                    | 7     | 5     | 2     | 0                           | 5     | 7     | 4     |

**II. RANDOM BUILDS: Let The Fates Decide (Random Difficulty)**

i. Shuffle all 22 Crucible Cards together.
ii. Blindly draw four cards for each player in the game.

## V. Determine the First Agekeeper

- Everyone rolls their Zodiac Die. The highest number becomes the first Agekeeper. Reroll ties.
- The first Agekeeper takes the Agekeeper's Key.
- The Key passes clockwise at the end of each Age.

### The Agekeeper's Responsibilities:

- **Set the Cosmic Age:** roll the Cosmic Age Die at the start of each Age and place it on the Sign bearing that number.
- **Manage the deck:** deal the Spring cards and manage the Kismeta discard pile.
- **Reset the Great Year Board:** prepare the play area for the next Age and pass the Agekeeper's Key clockwise.

## VI. Deal the Crucible Cards

The first Agekeeper finishes game prep. When this step is done, the first Cosmic Age begins.

- Shuffle the Crucible Deck; tap twice to cleanse.
- Deal four Crucible Cards **face up** to each player.
- Each player arranges them in a row along the edge of the Great Year Board. Every card's Formula is visible to all players from the start.
- The Agekeeper deals one face-up Minor Arcana card from the Kismeta Deck into each player's Spread. Redeal if a Major Arcana card is drawn.

The first Cosmic Age begins!

---

# ROUND OVERVIEW

Each Cosmic Age (round) moves through four Seasons in order: Spring, Summer, Autumn, and Winter. Play progresses clockwise around the table each Season, starting with the Agekeeper.

| SEASON    | WHAT HAPPENS                                                                 |
| --------- | ---------------------------------------------------------------------------- |
| 🌸 Spring | Sets the Age, deals new cards, and fixes your Sign for the Age.              |
| ☀️ Summer | Craft, trade, and act on your rivals to gain ground or slow someone down.    |
| 🍂 Autumn | Essence is paid out, Furnaces are fueled, and the Great Work moves forward.  |
| ⛰️ Winter | Closes the book on the Age and resets the table for the next one.            |

## ROUND AT A GLANCE

| **Season**                                                | **Step** | **Action**                                                                                                                                            |
| --------------------------------------------------------- | :------: | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 🌸 **SPRING** Set the Age, Deal the Cards & Build Your Tableau |  **1**   | **Set the Age:** the Agekeeper rolls the Cosmic Age Die. Read the Sign, Planet, and Element aloud. All players take the Gift of the Age.               |
|                                                           |  **2**   | **Set Your Zodiac Sign:** all players roll their Zodiac Die and move their Meeple to that Sign.                                                        |
|                                                           |  **3**   | **Deal the Kismeta Cards:** the Agekeeper deals 3 cards face down to each player.                                                                      |
|                                                           |  **4**   | **Build Your Tableau:** arrange your cards across your Spread, Hand, and Arcanum zones.                                                                |
|                                                           |  **5**   | **Card Lock:** cards are locked between your Hand and Spread until Winter.                                                                             |
| ☀️ **SUMMER** Craft, Distill, Trade & Contest             |          | _**CRAFT & DISTILL: TAKE ANY ACTION**_                                                                                                               |
|                                                           |  **1**   | **Craft an Astral House:** spend 3 cards matching your Sign's ruling Planet to build a House on your Sign.                                             |
|                                                           |  **2**   | **Upgrade to a Mansion:** spend 2 more cards of its Planet and place a Reagent from the supply beneath the House token.                                |
|                                                           |  **3**   | **Distill a Reagent:** spend 5 Essence of one Element to take the Reagent of that Element.                                                             |
|                                                           |          | _**CONTEST YOUR RIVALS: TAKE ANY ACTION**_                                                                                                           |
|                                                           |  **4**   | **Trade:** exchange Adept Cards, Astral Houses, Essence, Kismeta Cards, and Reagents freely.                                                           |
|                                                           |  **5**   | **Duel:** challenge a rival to a roll of the Zodiac Dice to steal a card from their Spread.                                                            |
|                                                           |  **6**   | **Extinguish Coal:** use a card effect to remove a rival's Coal from a Furnace and lower their Heat.                                                   |
|                                                           |  **7**   | **Siege the Forge:** pay an Ante of Reagents equal to the Shield Reagents guarding a rival's Stone in the Forge.                                     |
| 🍂 **AUTUMN** Harvest, Fuel & Conduct the Great Work      |  **1**   | **Harvest Elemental Essence:** score your Harvest Sources against the Aspects of the Age and take your Essence.                                        |
|                                                           |  **2**   | **Fuel Your Furnaces:** craft Coal and place it in the Furnace matching your Sign's Element to raise your Heat.                                        |
|                                                           |          | _**CONDUCT THE GREAT WORK**_                                                                                                                          |
|                                                           |  **3**   | **Fire the Stone:** complete an Alchemical Formula and send your Stone into the Forge. Commit up to 3 Shield Reagents.                               |
|                                                           |  **4**   | **Temper the Stone:** after a full Age in the Forge, advance your Stone to the next Mantle Ring space.                                                 |
|                                                           |  **5**   | **Rekindle the Stone ❄️:** pay 5 Essence to return a Stone from Stasis to its previous Forge position.                                                 |
| ⛰️ **WINTER** Final Actions of the Cosmic Age & Reset     |  **1**   | **Card Unlock:** move cards freely between your Hand and Spread.                                                                                       |
|                                                           |  **2**   | **Fateful Wager:** bet Cards and Reagents on the next Cosmic Age; double or lose your wager.                                                           |
|                                                           |  **3**   | **Enforce Card Limits:** Spread 7 cards \| Hand 5 cards \| Arcanum: 2 Adept cards.                                                                     |
|                                                           |  **4**   | **Transit the Age:** clear the board, shuffle the discard pile back in, and pass the Key clockwise.                                                    |

📌 _Return to this table during play as a quick reference._

---

## PHASE 1: 🌸 SPRING

### Set the Age, Deal the Cards & Build Your Tableau

Spring ushers in a new Cosmic Age. Set the Age, roll your own Sign, take your cards, and commit them to your zones for the Age ahead.

| **Step**                     | **Action**                                                                     |
| ---------------------------- | ------------------------------------------------------------------------------ |
| **1️⃣ Set the Age**           | The Agekeeper rolls the Cosmic Age Die. Read the Sign and its Aspects aloud.   |
| **2️⃣ Set Your Zodiac Sign**  | All players roll their Zodiac Die and move their Meeple to their Sign.         |
| **3️⃣ Deal the Kismeta Cards**| The Agekeeper deals 3 cards face down to each player.                          |
| **4️⃣ Build Your Tableau**    | Arrange your cards across your Spread, Hand, and Arcanum zones.                |
| **5️⃣ Card Lock**             | Cards are locked between your Hand and Spread until Phase 4: Winter.           |

### 1️⃣ Set the Age

- **Roll the Cosmic Age Die:** the Agekeeper rolls the 12-sided black die and places it on the Sign bearing that number.
- **Read the Aspects aloud:** announce the Sign's name, its ruling Planet, and its Element. These three are the **Aspects** of this Age, and every Harvest and Siege this Age is scored against them.
- **Gift of the Age:** all players take 1 Essence matching the Cosmic Age's Element.

📌 **Resolve Fateful Wagers:** if any player placed a Fateful Wager during the previous Winter, resolve it now against the Sign just rolled. See _Phase 4: Winter, Fateful Wager_.

### 2️⃣ Set Your Zodiac Sign

- **Roll your Zodiac Die:** each player rolls their color-matched 12-sided die.
- **Move your Meeple:** place it on the Sign you rolled on the Zodiac Wheel. This is your Sign for the Age.

Your Sign does three things. It is a Harvest Source, scored against the Aspects of the Age. It sets the one Sign where you may build an Astral House. And it sets the one Furnace you may fuel this Age.

### 3️⃣ Deal the Kismeta Cards

The Agekeeper deals **3 cards per player, face down**, from the Kismeta Deck.

⚠️ **If the Kismeta Deck runs out while dealing:** immediately shuffle the discard pile to form a new deck and continue. If there is no discard pile, the Fates intervene: all players return their Hand cards to the deck, the Agekeeper shuffles, and the deal continues.

### 4️⃣ Build Your Tableau

Your **Tableau** is everything you have in play, arranged across three zones: the Spread, the Hand, and the Arcanum. Arrange your cards between them now, and consider your goals for the Age as you do.

#### Spread

Cards you place in your Spread are face up and visible to every player, and their printed effects are active. Spread cards are the cards you craft with, the cards that pay your Attunements, and the cards that form your Stelliums. They also contribute to the Harvest in Autumn. Because they are out in the open, they are the only cards a rival can take from you in a Duel.

#### Hand

Cards you hold privately are your Hand cards. They are hidden, and their effects are inactive. Hand cards cannot be used in Crafting sets. They are safe from Duels, and you may commit them during a Siege.

#### Arcanum

Any Major Arcana card, Adept or Fate, is played into your Arcanum.

- **Fate Cards:** placed in your Arcanum and resolved at once, even if drawn as part of a face-down deal. Some carry an effect that lasts until Transit; the card states this. Return the card to the Kismeta discard pile at Transit.
- **Adept Cards:** purchased when drawn. Pay **3 Essence, of any type**, to add it to your Arcanum, or discard it. Adepts persist across Ages. You may hold two.

📌 **Note:** your Spread is your engine. Your Hand is your reserve: invisible to rivals, safe from Duels, and your ammunition in a Siege.

### 5️⃣ Card Lock 🔒

Once your Tableau is arranged, cards are locked in their zones until **Phase 4: Winter**. Locked cards may still be spent, crafted, traded, or committed as their own rules allow, but they cannot move between your Hand and your Spread.

Any new Minor Arcana card you gain in Summer or Autumn is added to your Spread. Major Arcana cards follow their normal rules.

📌 **Spring is complete.** The Age is set, your Sign is fixed, and your Tableau is built. Proceed to **Phase 2: ☀️ Summer**.

---

## PHASE 2: ☀️ SUMMER

### Craft, Distill, Trade & Contest

The Sun shines brightly as you craft your resources and consort with your rivals, bartering over trades, calling duels, and laying siege to exposed Stones.

Any of the following activities can be taken in Summer, depending on your goals for the Age. None are mandatory, and any can be taken in any order. Pass your turn when you are done.

| **Step**                   | **Action**                                                                                              |
| -------------------------- | ------------------------------------------------------------------------------------------------------- |
|                            | _**CRAFT & DISTILL: TAKE ANY ACTION**_                                                                 |
| **🏰 Craft Astral House**  | Spend 3 cards matching your Sign's ruling Planet to build a House on your Sign.                         |
| **🏛️ Upgrade to Mansion**  | Spend 2 more cards of its Planet and place a Reagent from the supply beneath the House token.           |
| **⚗️ Distill a Reagent**   | Spend 5 Essence of one Element to take the Reagent of that Element.                                     |
|                            | _**CONTEST YOUR RIVALS: TAKE ANY ACTION**_                                                             |
| **🤝 Trade**               | Exchange Adept Cards, Astral Houses, Essence, Kismeta Cards, and Reagents freely.                       |
| **🤺 Duel**                | Challenge a rival to a roll of the Zodiac Dice to steal a card from their Spread.                       |
| **💨 Extinguish Coal**     | Use a card effect to remove a rival's Coal from a Furnace and lower their Heat.                          |
| **⚔️ Siege the Forge**     | Pay an Ante of Reagents equal to the Shield Reagents guarding a rival's Stone in the Forge.             |

### 🏰 Craft Astral House

Astral Houses are permanent structures built on the Zodiac Wheel. Only one player may build on any given Sign, though other players may still roll onto that Sign and align with it.

#### How to Build an Astral House:

- **Spend 3 cards from your Spread** whose Planets match the ruling Planet of your current Sign.
- **Place your Astral House token** on the 🏰 icon on that Sign.

| 🏰 ASTRAL HOUSES     |                                                                                                                                                            |
| -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **BENEFIT**          | **HOW IT WORKS**                                                                                                                                           |
| Harvest Source       | Scored against the Cosmic Age every Autumn for the rest of the game. Each House is scored independently and stacks with all your other Harvest Sources.    |
| Attunement           | Fills one Attunement slot per Firing, for any Aspect the House carries, and is not spent doing so.                                                          |
| Permanence           | Once placed, a House stays on the Sign where it was built and keeps working no matter what your Zodiac Die shows in later Ages. Ownership can change hands through Trade, but the House itself never moves. |

### 🏛️ Upgrade to a Mansion

Once a House is built, you may upgrade it to an **Astral Mansion**:

- **Spend 2 more cards** of that Sign's ruling Planet from your Spread.
- **Place a Reagent token from the supply** beneath the House token to mark the upgrade.

That Reagent is a marker, not a resource: it is never spent from your own stock, never enters your Tableau, and returns to the supply only if the Mansion is ever lost. If a Mansion changes hands through Trade, its Reagent marker and its doubled output go with it. A Mansion doubles its Alignment score at the Harvest and in a Siege.

### ⚗️ Distill a Reagent

Reagents are refined from Essence, and they pay your Alchemical Formulas, shield your Stone, and pay Siege Antes.

- **Spend 5 Essence of one Element** to the bank.
- **Take 1 Reagent** of that Element from the supply.

Distilling is available freely on your turn in Summer, Autumn, and Winter. See the _Correspondence_ table in the Appendix for which Element yields which Reagent.

💡 **Example:** you spend 5 Fire Essence and take 1 Sulfur.

### 🤝 Trade

Trade freely with any rival on your turn.

| CAN TRADE      | CANNOT TRADE        |
| -------------- | ------------------- |
| Adept Cards    | Crucible Cards      |
| Astral Houses  | Fate Cards          |
| Essence        | Coal                |
| Kismeta Cards  | Stone positions     |
| Reagents       | Zodiac Die results  |

### 🤺 Duel

Challenge a rival to a roll of the Zodiac Dice to steal a card from their Spread.

#### How to Declare a Duel:

- **Declare:** name your opponent and the specific card you are targeting in their Spread.
- **Check Spreads:** both players review their Spreads for card effects that apply to Duels.
- **Roll:** both players roll their Zodiac Dice. The highest roll wins, and the defending player wins ties.
- **Resolve:** see the outcomes below.

| DUEL RESULTS      |                                                                                                     |
| ----------------- | --------------------------------------------------------------------------------------------------- |
| **RESULT**        | **OUTCOME**                                                                                         |
| Challenger Wins   | The challenger takes the targeted card from the defender's Spread, and may challenge them again.    |
| Challenger Loses  | The defender keeps the card. The challenger cannot challenge that same defender again this Age.     |

📌 **Note:** several Minor Arcana carry Attack, Defend, and Ward effects that shift Duel rolls. Check both Spreads before rolling.

### 💨 Extinguish Coal

Certain cards let you extinguish a rival's Coal, removing it from a Furnace and lowering their Heat by 1 in that Element. The Coal returns to its owner's personal supply.

Other cards Ward your Coal against being extinguished. Plan accordingly: a rival who extinguishes the right Coal at the right moment can put a Formula out of reach for an entire Age.

### ⚔️ Siege the Forge

_Attack a rival's Stone while it lies exposed._

A Stone is vulnerable while it sits in the Forge. It is exposed for a single Summer, between the Autumn when its owner Fires it and the Autumn when they Temper it.

#### Shield Reagents

When you Fire your Stone, commit up to 3 Reagents as **Shield Reagents**. A rival must match them to attack, so a Stone without them stands unguarded.

#### To Declare a Siege

- Choose your target and pay the **Ante** (Reagents equal in number to the Shield Reagents guarding the Stone, in any combination of Elements).
- Each Alchemist may declare one Siege against a given Stone per Age.

#### Resolve the Siege

1. **Opening Score.** Both players total their current Alignment against the current Cosmic Age (Zodiac Die, Astral Houses, Adept Cards, and Spread Cards).
2. **Commit from the Hand.** Defender adds Hand cards face down, then Attacker does the same.
3. **Concede or Continue.** Either player may back out once Hand cards are committed. A conceding Defender moves their Stone to Stasis. A conceding Attacker cannot declare another Siege against this Stone for the rest of its current Transmutation Stage. Nothing committed is lost or burned.
4. **If Neither Concedes.** Reroll the Cosmic Age Die for a new Siege Age. Turn Hand cards face up, tally again, and the highest total wins (Defender wins ties). The loser burns every Hand card they committed; the winner returns theirs to their Hand.

| SIEGE OUTCOMES |                                                                                                                                                                     |
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **RESULT**     | **OUTCOME**                                                                                                                                                         |
| Attacker Wins  | The Defender's Stone is Seized and moves to their Stasis (❄️) zone. Its Shield Reagents are lost to the supply, and the Attacker takes back their Ante. From the Autumn of the next Age, the Defender may pay 5 Essence to Rekindle the Stone. |
| Defender Wins  | The Stone holds and continues its Transmutation. Shield Reagents remain intact, and the Attacker's Ante is lost to the supply. The Defender gains a **Besieged Bonus** of +2 for any later Siege against this Stone in its current Stage. |

---

## PHASE 3: 🍂 AUTUMN

### Harvest, Fuel & Conduct the Great Work

As the Cosmic Age shifts into Autumn, the heavens pay out, the Furnaces are stoked, and Stones move. The activities available to you depend on where your Stone stands and what you managed to assemble this Age. Take any action available to you, in the order you see fit.

| **Step**                     | **Action**                                                                                        |
| ---------------------------- | ------------------------------------------------------------------------------------------------- |
| **1️⃣ Harvest Essence**       | Score your Harvest Sources against the Aspects of the Age and take your Essence from the bank.    |
| **2️⃣ Fuel Your Furnaces**    | Place Coal into the Furnace matching your Sign's Element to raise your Heat.                      |
|                              | _**CONDUCT THE GREAT WORK**_                                                                      |
| **3️⃣ Fire the Stone 🔥**     | Complete an Alchemical Formula and send your Stone into the Forge. Commit up to 3 Shield Reagents. |
| **4️⃣ Temper the Stone ⏳**   | After a full Age in the Forge, advance your Stone to the next Mantle Ring space.                  |
| **5️⃣ Rekindle the Stone ❄️** | Pay 5 Essence to return your Stone from Stasis to its previous Forge position.                    |

### 1️⃣ Harvest Elemental Essence

You earn Essence by aligning the items in your Tableau with the Aspects of the Cosmic Age, set back in Spring.

#### Harvest Sources

Your Harvest Sources are your **Zodiac Die**, your **Astral Houses**, your **Adept Cards**, and your **Spread Cards**. Compare each source to the Cosmic Age and find matching Aspects. The Essence earned from each match is shown on the Zodiac Wheel.

#### Counting the Harvest

1. **Tally your Harvest.** Compare the Aspects of each Harvest Source (Sign, Planet, and Element) to the Aspects of the Cosmic Age. Scores for matching Aspects are found on the Zodiac Wheel of the Great Year Board. Each point pays 1 Essence.
2. **Stack every match.** If a source matches more than one Aspect, add every tier it matches. A source matching all three at once, Sign, Planet, and Element together, scores the full stack: a **Perfect Match**.
3. **Collect your Harvest.** Take your full total from the bank, in Essence of the Cosmic Age's Element.

| 🍂 HARVEST SCORING                                              |                 |
| --------------------------------------------------------------- | --------------- |
| **ASPECT MATCHED**                                              | **ESSENCE**     |
| Sign                                                            | +3              |
| Planet                                                          | +2              |
| Element                                                         | +1              |
| Perfect Match (all three at once)                               | +6              |
| No Match                                                        | +0              |

| 💡 EXAMPLE: SCORPIO AGE (Aspects = Scorpio, Mars, Water) |                                |             |
| -------------------------------------------------------- | ------------------------------ | ----------- |
| **SOURCE**                                               | **ASPECT MATCHED**             | **ESSENCE** |
| Zodiac Die: Scorpio                                      | Sign match (Scorpio), Perfect Match | +6     |
| Spread: 7 of Pentacles                                   | Planet match (Mars)            | +2          |
| Astral House: Pisces                                     | Element match (Water)          | +1          |
| Adept Card: The Hermit                                   | Sign match (Scorpio), Perfect Match | +6     |
| **TOTAL ESSENCE INCOME**                                 |                                | **15 Water Essence** |

### 2️⃣ Fuel Your Furnaces

_Raising the Heat of your Furnaces to prepare for a Transmutation Stage._

The four Furnaces prepare your Stone for each Transmutation Stage. Each Furnace holds three Coal, shared among all players and claimed first come, first served. Coal is both crafted and placed here in Autumn.

1. **Craft your Coal.** Spend 3 cards of one suit from your Spread, matching the Element of the Furnace you may fuel, to craft 1 Coal, per your Codex. Coal you craft but do not place stays in your personal supply.
2. **Place your Coal** in the Furnace matching the Element of your Sign for this Age. That is the only Furnace you may fuel.
3. **Raise your Heat.** Each Coal you place raises your own **Heat** by +1 in that Element. Heat is tracked separately for each player and counts only the Coal they own, so two rivals burning in the same Furnace each read their own level. Every Coal is in its owner's player color, so your fuel stays identifiable on a crowded board.

| COAL YOU OWN IN THIS FURNACE | YOUR HEAT LEVEL          |
| ---------------------------- | ------------------------ |
| 1                            | 1 (Kindled)              |
| 2                            | 2 (Ablaze)               |
| 3                            | 3 (Radiant, the maximum) |

💡 **Example:** your Sign this Age is Leo, a Fire Sign, so the Fire Furnace is the one you may fuel. You spend a 4 of Wands, a 7 of Wands, and the King of Wands from your Spread to craft 1 Coal, and place it in the Fire Furnace. Your Heat in Fire is now Kindled.

📌 **Note:** because a Furnace holds only three Coal in total across all players, a rival who fills it first can lock you out of the Heat you need. Watch the Furnace matching your Sign as soon as Spring ends.

### 3️⃣ Fire the Stone 🔥

_Starting a Transmutation Stage. Available while your Stone is in position 0, 2, 4, or 6._

1. Complete an Alchemical Formula, paying its Attunement, its Reagents, and meeting its Furnace Heat, to send your Stone into the Forge.
2. Commit up to 3 Reagents as **Shield Reagents** beside your Stone in the Forge.
3. A Transmutation Stage takes one full Age. Your Stone is exposed to Siege for the single Summer it spends in the Forge.
4. The Forge holds one station for each Stage: Lead, Bronze, Silver, and Gold. Every Alchemist competes for the same four, so a station occupied by a rival's Stone is closed to you until it is free.

#### The Alchemical Formula

Each Crucible Card shows three requirements you must satisfy at once before Firing your Stone.

| CRUCIBLE CARDS: ALCHEMICAL FORMULAS |                                                                                                                                                                  |
| ----------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **REQUIREMENT**                     | **WHAT IT REQUIRES**                                                                                                                                             |
| Attunement                          | A count of Element and Planet slots. Fill them from your Spread cards, which are spent, or from an Astral House, which fills one slot per Firing and is not spent. |
| Reagents                            | Specific Reagent types and quantities, discarded to the supply when you Fire.                                                                                     |
| Heat                                | A level of Heat you must hold in a named Furnace, or in two Furnaces for a Gold Stage card. The Coal stays in place until you Temper.                             |

### 4️⃣ Temper the Stone ⏳

_Completing a Transmutation Stage. Available while your Stone is in position 1, 3, 5, or 7._

1. If your Stone has remained in the Forge for one full Age, advance it to the next space on the Mantle Ring.
2. Set aside the completed Alchemical Formula card. Consume the Coal used in this Transmutation Stage, removing it from the Furnaces and lowering your Heat.

⚠️ **Important:** a Stone that returned from Stasis this Age cannot Temper. It must sit in the Forge for a full Age first.

### 5️⃣ Rekindle the Stone ❄️

_Returning to the Forge after Stasis. Available while your Stone is in a Stasis zone ❄️._

1. From the Autumn of the Age after your Stone was Seized, pay **5 Essence, of any type**, to return it to its previous position in the Forge.
2. If that position is occupied, you must wait until it is free.

### Crucible Positions Reference: Your Stone's Current Position

- When your Stone is on the **Mantle Ring** (even positions: 0, 2, 4, 6), it is resting. It is safe from Siege but does not advance. While there, work toward the next Alchemical Formula.
- When you **Fire** a Crucible Card, your Stone moves into the **Forge**, where it works through one Transmutation Stage. It is exposed to Siege for the single Summer it spends there. After a full Age, you may Temper it to the next Mantle Ring position or to the Altar of Kismeta.
- If your Stone is in a **Stasis zone** (marked ❄️), it has stalled. Pay 5 Essence next Autumn to Rekindle it.

| POS. | LOCATION         | STATE     | SAFE? |
| ---- | ---------------- | --------- | ----- |
| 0    | Mantle Ring      | RESTING   | YES   |
| 1    | Forge (Lead)     | FORGING   | NO    |
| 2    | Mantle Ring      | RESTING   | YES   |
| 3    | Forge (Bronze)   | FORGING   | NO    |
| 4    | Mantle Ring      | RESTING   | YES   |
| 5    | Forge (Silver)   | FORGING   | NO    |
| 6    | Mantle Ring      | RESTING   | YES   |
| 7    | Forge (Gold)     | FORGING   | NO    |
| 8    | Altar of Kismeta | Victory.  | YES   |

#### Philosopher's Stone States

| STATE     | POSITION                 | WHAT IT MEANS                                                                                                                              |
| --------- | ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------- |
| Resting   | Mantle Ring (0, 2, 4, 6) | Safe. Cannot be Sieged. Work toward your next Alchemical Formula.                                                                          |
| Forging   | Forge (1, 3, 5, 7)       | Transmutation underway. Exposed to Siege for one Summer. Protect it with Shield Reagents.                                                  |
| In Stasis | ❄️ Stasis zone           | Seized after a lost Siege. Pay 5 Essence in Autumn to Rekindle it to its previous Forge position.                                          |
| Complete  | Altar of Kismeta         | You have finished all four Transmutation Stages and won the game. All hail! All hail! The Magnus Alchemist!                                 |

---

## PHASE 4: ⛰️ WINTER

### Final Actions & Reset

Winter closes the Cosmic Age. Unlock your cards, place a wager on what the heavens will bring, enforce card limits, then Transit the Age.

| **Step**                   | **Action**                                                                                          |
| -------------------------- | ---------------------------------------------------------------------------------------------------- |
| **1️⃣ Card Unlock**         | Move cards freely between your Hand and Spread.                                                      |
| **2️⃣ Fateful Wager**       | Bet Cards and Reagents on the next Cosmic Age; double or lose your wager.                            |
| **3️⃣ Enforce Card Limits** | Spread 7 cards \| Hand 5 cards \| Arcanum: 2 Adept cards.                                            |
| **4️⃣ Transit the Age**     | Clear the board, shuffle the discard pile back into the deck, and pass the Key clockwise.            |

### 1️⃣ Card Unlock 🔓

Cards are now unlocked. Rearrange them freely between your Hand and your Spread before taking any other Winter action. Distilling is also available on your turn, and Winter is a natural moment to refine before limits are enforced.

### 2️⃣ Fateful Wager 🃏

_(OPTIONAL)_ Before the Age ends, any player may wager on which Zodiac Sign will rule the next Cosmic Age. Guess correctly and your wager is doubled. Guess wrong and it is lost to the Fates.

#### How to Place a Wager:

- **Choose a Sign:** declare any Zodiac Sign as your prediction for the next Cosmic Age.
- **Commit your stake:** place any number of Cards and Reagents on that Sign. Place your Meeple on top to identify your bet.
- **Resolution:** wagers are resolved at the start of the next Spring, once the Agekeeper sets the new Age.

| FATEFUL WAGER RESULTS          |                                                                                       |
| ------------------------------ | ------------------------------------------------------------------------------------- |
| **RESULT**                     | **OUTCOME**                                                                           |
| Correct: Sign matches          | Your wager is doubled and returned to you.                                            |
| Incorrect: Sign does not match | Losing Reagents return to the supply, and losing Cards go to the Kismeta discard pile. |

### 3️⃣ Enforce Card Limits

Discard down to these limits before continuing.

| CARD LIMITS |               |                                                                    |
| ----------- | ------------- | ------------------------------------------------------------------ |
| **ZONE**    | **MAX LIMIT** | **NOTES**                                                          |
| Spread      | 7 cards       | Discard excess cards of your choice down to 7.                     |
| Hand        | 5 cards       | Discard excess cards of your choice down to 5.                     |
| Arcanum     | 2 Adept cards | Fate Cards are returned to the discard pile at Transit.            |

### 4️⃣ Transit the Age

- **Return Fate Cards:** all Fate Cards go to the Kismeta discard pile, and any effect lasting "until Transit" now ends.
- **Clear the board:** all players clear their Zodiac Dice and Meeples.
- **Shuffle and pass:** the Agekeeper shuffles the Kismeta discard pile back into the Kismeta Deck, then passes the Agekeeper's Key and the Cosmic Age Die clockwise to the next player.
- **Start the next Age:** return to Phase 1: Spring.

---

# WINNING THE GAME

The first player to advance their Stone to the Altar of Kismeta at the center of the Great Year Board wins the game. You will have completed all four of your Alchemical Formulas and passed through all four Transmutation Stages, successfully transmuting your Lead into Gold.

Once your Stone reaches Gold in the Forge (position 7), you are one Age from winning, and one Summer from your most dangerous moment. Survive that Summer's Sieges and you may Temper to the Altar of Kismeta (position 8) in Autumn.

If two players would reach the Altar in the same Autumn, the one earlier in turn order from the Agekeeper wins.

All Hail! All Hail!
The Magnus Alchemista Grandis Illuminatus ex Kismeta!

---

# QUICK TIPS

Keep these principles in mind as you find your footing across the first few Ages.

1. **Your Alignments Set Your Income.** Every Essence you earn comes from matching your Harvest Sources against the three Aspects of the Cosmic Age. A source that matches all three at once is worth six times a bare Element match, so chase Perfect Matches wherever you can engineer them.

2. **Your Spread Is Your Engine.** Spread cards have active effects, form Stelliums, craft Coal and Astral Houses, pay Attunements, and score at the Harvest. Hand cards do almost none of that. What is visible is what works, and what is visible is also what a rival can take.

3. **Watch the Furnace, Not Just the Formula.** A Furnace holds only three Coal for the whole table. If your Formula needs Radiant Heat, you need all three spaces, and every rival who fuels that Furnace first has taken one away from you. Fuel early in an Age when your Sign gives you access.

4. **Your Sign Gates Everything for the Age.** Your Zodiac Die roll decides which Furnace you may fuel and where you may build a House. When your roll opens the Furnace you need, drop everything and fuel it. That window may not come again for several Ages.

5. **Astral Houses Compound.** A House is a Harvest Source every Autumn for the rest of the game, and it fills an Attunement slot every Firing without ever being spent. Upgrade to a Mansion when you can: the doubled Alignment pays off at every Harvest and in every Siege. A House built in the first Age pays out over and over. A House built in the last Age barely pays at all.

6. **Distill Before You Need It.** Reagents pay Formulas, declare Siege Antes, and shield your Stone in the Forge. Essence sitting in your bank does none of those things. Convert steadily at 5:1 rather than in a panic.

7. **Adept Cards Are Permissions.** An Adept never hands you a bonus, it lifts a restriction: fuel any Furnace, build on the Cosmic Age, ignore Card Lock, Siege without paying an Ante. Pick the one that unlocks the part of your engine that is jammed, and consider which Sign it keys to, because a Resonant Adept is dramatically stronger. Each costs 3 Essence of any type.

8. **Time Your Entry into the Forge.** Firing is a commitment: your Stone is exposed for exactly one Summer. Fire in an Age when your rivals are short on Reagents, or when your Hand is deep enough to win the Siege that follows. Shield Reagents are your first line of defense; a deep Hand is your second.

---

# STRATEGY GUIDE

## 1. Two Economies, One Bottleneck

Kismeta runs a dice economy and a card economy side by side, and your Crucible Cards demand payment in both.

### The Dice Economy

Your Zodiac Die, Astral Houses, Adept Cards, and Spread Cards pay out Essence each Autumn based on how well they align with the Age. Essence distills into Reagents at a fixed 5:1 rate. This economy is broad and forgiving: almost anything you own contributes something, and a strong Alignment can pay out fifteen Essence in a single Autumn.

### The Card Economy

Cards convert into Coal (3 of one suit) and Astral Houses (3 of one Planet). This economy is narrow and brutal. You receive only three cards per Age, plus what you steal, draw from effects, or trade for. Every card spent on a Coal is a card not spent on a House, an Attunement, or a Stellium.

### Where They Meet

An Alchemical Formula needs Attunement (cards), Reagents (Essence), and Heat (Coal, which is also cards). Cards are the binding constraint in nearly every game. Players who over-invest in Essence find themselves rich and unable to Fire.

## 2. The Sign Lottery and How to Beat It

Your Zodiac Die result each Spring decides which Furnace you may fuel and which Sign you may build on. That is a hard gate on your two most important long-term investments, and it is entirely random.

Three tools bend it:

- **The Mercury Stellium** lets you fuel any Furnace, regardless of your Sign.
- **The Jupiter Stellium** lets you build an Astral House on the **Cosmic Age**, not only your own Sign.
- **The Chariot** (Adept, Leo) lets you shift your Zodiac Die result one Sign in either direction on the Wheel, once per Age.
- **Strength** (Adept, Sagittarius) lets you declare a Siege without paying the Ante and win any tie in Sieges.

Any one of these converts your engine from reactive to deliberate. Acquiring one early is often worth more than a full Age of Harvest income.

## 3. Spread Versus Hand

The Card Lock freezes cards between your Spread and Hand from Spring until Winter, so the arrangement you commit to in Spring is the arrangement you live with for the entire Age.

- **Over-commit to your Spread** and you have a powerful, visible engine and nothing held back for a Siege, with every card exposed to Duels.
- **Over-commit to your Hand** and you are safe, quiet, and unable to craft, attune, or harvest.

The right split depends on where your Stone is. Resting on the Mantle Ring, load your Spread and build. About to Fire, hold cards back: your Hand is the only thing that wins the Siege that follows.

## 4. The Forge Window

The path to victory is a push-your-luck curve. Your Stone is safe on the Mantle Ring and safe at the Altar. It is vulnerable for exactly one Summer per Stage, four Summers across a whole game.

A Siege costs the attacker an **Ante of Reagents equal to the Shield Reagents** guarding the Stone, so the window to attack is gated by how well the defender invested in protection. A successful Siege Seizes your Stone to Stasis, where Rekindling costs **5 Essence** and an entire Age. A won Siege is close to free for the attacker (they take their Ante back) and expensive for you, which means the defense is not purely economic: a deep Hand your rivals cannot see, committed at the right moment, plus the **Besieged Bonus** of +2 stacking after each successful defense.

The counter-pressure runs the other way. A failed Siege costs the attacker their Ante outright (lost to the supply), so attackers cannot swing freely against well-shielded Stones.

## 5. Denial

Kismeta gives you three ways to hurt a rival without advancing yourself.

- **Duels** take a specific card from a Spread. The best targets are not the highest-value cards but the load-bearing ones: the third card of a Stellium, or the last card of an Attunement.
- **Extinguish** removes a rival's Coal and drops their Heat. Timed just before their Autumn, it can push a Formula out of reach for a whole Age.
- **Siege** stalls a Stone for an Age.

Each costs you tempo. The judgment call is whether a leader is close enough to the Altar that stalling them is worth an Age of your own progress.

## 6. Game Modes & Depth

The Crucible Deck build decides how heavy the game is.

### Quickplay (Low-to-Medium Depth)

Weighted to Groups A and B: small single-Element Attunements, light Reagent costs, and Heat in one Furnace. Teaches the core loop without the Attunement puzzle.

### Standard Game (Medium-High Depth)

A balanced spread across the Groups. Full cognitive load: Card Lock, House placement, Furnace competition, and the Forge window.

### Magnus Alchemist (Expert Depth)

Weighted to Groups C and D. Group C demands complex Attunements and card straights. Group D demands the heaviest Reagent payments and Heat spread across multiple Furnaces at once. Group D Formulas require multi-Age planning, Astral Houses placed Ages in advance, and usually a Stellium or an Adept to lift the Furnace restriction.

## Conclusion & Verdict

Kismeta offers a robust competitive environment. Luck is present in the dice, but it acts as changing terrain rather than the arbiter of victory. A skilled player consistently outperforms an unskilled one by planning Attunements several Ages ahead, placing Astral Houses early, timing their entry into the Forge, and keeping enough hidden in Hand to survive the Summer that follows.

It rewards forward planning, flexibility, and sharp defensive management.

---

# APPENDIX

## LORE: PROLOGUE ~ Alchemists Of The Great Year

Long ago, alchemists sought to uncover the divine mysteries of the Cosmos, whispering secrets to the Fates and shaping matter to their will. Within the fiery Crucible of Kismeta, they transformed scarcity into abundance, ushering in an era of prosperity. Their lives were filled with wealth, wonder, and limitless potential, marking a golden age for humanity.

At the core of their quest was the Philosopher's Stone, a symbol of cosmic perfection and the key to immortality. Alchemists labored over crucibles and cauldrons for ages, each striving to claim the Stone's elusive power. Yet, despite their brilliance and dedication, the Stone remained just beyond reach.

The Fates fell silent, and the Golden Age faded into history as the Great Year turned again. Cosmic energy waned, drawing consciousness into darkness, beginning a long, treacherous Dark Age. The tales of the Alchemist's Great Work and the Philosopher's Stone became myths and folklore.

But now, the Fates whisper again. Their voices echo through time, summoning the brightest minds to the Great Hall of the Fates, a sacred cosmic convergence outside of time where destinies are forged, and rivalries burn bright. Across the Cosmic Ages of the Great Year, you have been called once more to accomplish the Great Work that your forebears were unable to.

The first among you to reach the Altar of Kismeta with your Philosopher's Stone intact will be proclaimed Magnus Alchemista Grandis Illuminatus ex Kismeta, and guide humanity into a new Golden Age.

Move swiftly, dear Alchemist.

The Fates beckon.

The Crucible of Kismeta awaits.

---

## The Zodiac Wheel

Every Sign on the Zodiac Wheel carries three Aspects: itself, its ruling Planet, and its Element. Each Sign also carries a number from 1 to 12, which is how the Cosmic Age Die and your Zodiac Die select it.

| # | SIGN         | RULING PLANET | ELEMENT | CARD SUIT     |
| - | ------------ | ------------- | ------- | ------------- |
| 1 | Aries        | Mars          | Fire    | 🪄 Wands      |
| 2 | Taurus       | Venus         | Earth   | 🪙 Pentacles  |
| 3 | Gemini       | Mercury       | Air     | ⚔️ Swords     |
| 4 | Cancer       | Moon          | Water   | 🍷 Cups       |
| 5 | Leo          | Sun           | Fire    | 🪄 Wands      |
| 6 | Virgo        | Mercury       | Earth   | 🪙 Pentacles  |
| 7 | Libra        | Venus         | Air     | ⚔️ Swords     |
| 8 | Scorpio      | Mars          | Water   | 🍷 Cups       |
| 9 | Sagittarius  | Jupiter       | Fire    | 🪄 Wands      |
| 10 | Capricorn   | Saturn        | Earth   | 🪙 Pentacles  |
| 11 | Aquarius    | Saturn        | Air     | ⚔️ Swords     |
| 12 | Pisces      | Jupiter       | Water   | 🍷 Cups       |

**Harvest values** for each Aspect are printed on the Zodiac Wheel: Sign +3, Planet +2, Element +1, and a Perfect Match (all three at once) scores the full stack of +6. Each point pays 1 Essence.

📌 **Note on valid pairs:** the table above is also the list of legal Planet and Element combinations. A Crucible Card in Group C calls for a pair that appears here. A Group D card calls for a pair that does not, such as Sun with Water, which no single Sign carries. That is why Gold Stage Formulas cannot be satisfied from your Zodiac Die alone.

## Correspondence

Every Sign, card suit, Furnace, Reagent, and Essence corresponds to one of the four Elements. The color of each item shows what it is compatible with.

| CORRESPONDENCE: ELEMENTS, ZODIAC SIGNS, CARD SUITS, REAGENTS & FURNACES |                                |               |               |             |
| ------------------------------------------------------------------------ | ------------------------------ | ------------- | ------------- | ----------- |
| **ELEMENT**                                                              | **ZODIAC SIGNS**               | **CARD SUIT** | **REAGENT**   | **FURNACE** |
| Fire                                                                     | Aries, Leo, Sagittarius        | 🪄 Wands      | Sulfur        | 🔴 Red      |
| Earth                                                                    | Taurus, Virgo, Capricorn       | 🪙 Pentacles  | Vitriol       | 🟢 Green    |
| Air                                                                      | Gemini, Libra, Aquarius        | ⚔️ Swords     | Quicksilver   | 🟡 Yellow   |
| Water                                                                    | Cancer, Scorpio, Pisces        | 🍷 Cups       | Aqua Regia    | 🔵 Blue     |

A Furnace's color describes the Furnace itself, never its occupants. Coal is always in its owner's player color, so a green Coal sitting in the red Furnace is simply an Alchemist fueling Fire.

## KISMETA CARDS

The Kismeta Deck (blue back) holds 134 cards: 112 Minor Arcana and 22 Major Arcana, shuffled together as one deck.

### Minor Arcana

112 cards, the building blocks of the Great Work: 28 per suit. Each card carries four traits:

- **Rank:** Ace, 2 through 10, Princess, Knight, Queen, King.
- **CV:** the card's numeric value, from 1 for an Ace to 14 for a King.
- **Suit:** Cups, Pentacles, Swords, or Wands, each tied to an Element and a Reagent.
- **Planet:** one of seven ruling Planets. Six of them anchor a Stellium, formed by holding three cards of that Planet in your Spread. The seventh, the Sun, anchors no Stellium and is wild instead.
- **Effect:** printed on the card, active only while it sits in your Spread.

Each suit runs the same ladder of effects, expressed in that suit's Element. Every rank appears twice per suit, except the 7, which has two distinct single-copy versions.

📌 **For the complete Minor Arcana reference** — every rank, effect, and copy count across all four suits — see **[Kismeta_CardReference.md](Kismeta_CardReference.md)**.

### Major Arcana

22 cards: 10 Fates and 12 Adepts.

**Fate Cards** resolve the moment they are drawn: place them in your Arcanum and apply their effect at once. Some carry a duration that lasts until Transit. Return them to the Kismeta discard pile at Transit. Fates carry no Aspects and cost nothing.

**Adept Cards** are purchased when drawn: pay **3 Essence, of any type**, to add them to your Arcanum, or discard them. You may hold two. Every Adept has two tiers: its **Base** text always applies, and it becomes **Resonant** whenever the Cosmic Age shares one of its three Aspects (Sign, Planet, or Element).

📌 **For the complete Fate and Adept card reference**, see **[Kismeta_CardReference.md](Kismeta_CardReference.md)**.

## Stelliums

Every Minor Arcana card carries a Planet. Hold **three cards of one Planet** together in your **Spread** to activate that Planet's **Stellium**. The effect holds only while all three remain: losing one to a Duel switches it off.

Only Spread cards count. Hand cards and Arcanum cards never contribute to a Stellium. An Ace is wild for any one Stellium. One Ace per Stellium.

| Planet        | Ranks         | While Active                                                          |
| ------------- | ------------- | --------------------------------------------------------------------- |
| ☽ **Moon**    | 2, Queen      | Your Coal cannot be Extinguished.                                     |
| ☿ **Mercury** | 5, Princess   | You may fuel any Furnace, not only the one matching your Sign.        |
| ♀ **Venus**   | 4, 9          | At Harvest, take your Essence in any one Element you choose.          |
| ♂ **Mars**    | 7, Knight     | Losing a Duel does not bar you from challenging that rival again this Age. |
| ♃ **Jupiter** | 3, 8, King    | You may build an Astral House on the **Cosmic Age**, not only your own Sign. |
| ♄ **Saturn**  | 6, 10         | Your Spread limit is 9 instead of 7.                                  |

📌 **For the full Stellium Codex and suit-specific card effects**, see **[Kismeta_CardReference.md](Kismeta_CardReference.md)**.

## Card Effect Verbs

_Shared rules language printed on Minor and Major Arcana cards._

| VERB            | MEANING                                                                                                                              |
| --------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| **Burn**        | Discard this card to use the effect that follows.                                                                                    |
| **Distill**     | Spend 5 Essence of one Element to take the Reagent of that Element.                                                                  |
| **Extinguish**  | Remove one Coal from a Furnace. It returns to its owner's supply and their Heat in that Element drops by 1. A Ward can stop it.      |
| **Neutralize**  | Cancel the Reversed effect of a card in your Spread. Both cards remain in play.                                                      |
| **Reversed**    | A penalty that applies only while this card is in your Spread. Moving it to your Hand, or Neutralizing it, ends the penalty.         |
| **Scry X**      | Look at the top X cards of the Kismeta Deck, or a rival's Hand, as the card states. What you learn is yours to keep.                 |
| **Sever**       | Switch off one card's printed text until Transit, as the card states.                                                                |
| **Ward**        | The warded card, Stone, or resource cannot be targeted, as the card states.                                                          |

## Crucible Cards

The Crucible Deck (red back) holds 22 cards across four Groups. Each card is one Transmutation Stage, and its face shows a full Alchemical Formula: an Attunement, a Reagent cost, and a Furnace Heat requirement.

**Attunement** is paid from Spread cards, which are spent, or from an Astral House, which fills one slot of any Aspect it carries, once per Firing, and is not spent.

### Group Structure

| GROUP | STAGE  | DIFFICULTY | CARDS IN DECK | TYPICAL DEMANDS                                      |
| ----- | ------ | ---------- | ------------- | ---------------------------------------------------- |
| A     | Lead   | Beginner   | 4             | 2-slot Attunement, 2 Reagents, Heat in 2 Furnaces    |
| B     | Bronze | Standard   | 7             | 3-slot Attunement, mixed Reagents, Heat in 3 Furnaces |
| C     | Silver | Advanced   | 7             | 3-card straights, heavier Reagents, mixed Heat       |
| D     | Gold   | Mastery    | 4             | Four Courts Attunement, heaviest Reagents, Heat in 4 Furnaces |

Each Formula advances your Philosopher's Stone through one Transmutation Stage and calls for three things:

- **Attunement:** a count of Element and Planet slots, paid from your Spread cards, which are spent, or from your Astral Houses, which are not.
- **Reagents:** a set quantity of Reagent tokens.
- **Heat:** a required level of Furnace Heat, in one Furnace or in several.

Your four Formulas sit face up from the start of the game, so every rival can see what you are building toward, and you can see theirs.

📌 **For the full Crucible Card reference** — every card's Attunement, Reagents, and Heat — see **[Kismeta_CardReference.md](Kismeta_CardReference.md)**.

## Tableau: Spread, Hand & Arcanum Zones

You always manage your Tableau, everything you have in play, across three zones.

| ZONE    | CARD TYPE                                     | VISIBILITY | EFFECTS  | MAX LIMIT                                    | USED FOR                                                                                                       | CANNOT BE USED FOR                          |
| ------- | --------------------------------------------- | ---------- | -------- | -------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | ------------------------------------------- |
| SPREAD  | MINOR ARCANA: Cups, Pentacles, Swords & Wands | Visible    | Active   | 7 cards, enforced in Winter (9 with Saturn)  | Paying Attunements, Crafting Coal, Building Astral Houses, Forming Stelliums, Harvest scoring, Trading, Wagers, Sieges | Nothing. Spread cards are eligible for every use, and are the only cards a rival may take in a Duel. |
| HAND    | MINOR ARCANA: Cups, Pentacles, Swords & Wands | Hidden     | Inactive | 5 cards, enforced in Winter                  | Committing in a Siege, Trading, Wagers                                                                         | Crafting sets, Attunements, Harvest scoring, Duel targets |
| ARCANUM | MAJOR ARCANA: Adept & Fate Cards              | Visible    | Active   | 2 Adepts (3 with the Hierophant); Fates until Transit | Adept effects and Aspects (Harvest scoring), Trading Adepts                                          | Crafting sets, Attunements, Duel targets, Wagers |

---

# GLOSSARY OF TERMS

_Terms are listed alphabetically. Each entry provides a definition only. For full rules, see the referenced Season or section._

| Term | Definition |
| :--- | :--------- |
| **Adept Card** | A purchased Major Arcana card. Pay 3 Essence, of any type, when it is drawn to add it to your Arcanum, or discard it. It persists across Ages and counts as a Harvest Source. Limit two. |
| **Age** | One complete cycle of play, made of Spring, Summer, Autumn, and Winter. An Age is what other games would call a round. |
| **Agekeeper** | The player holding the Agekeeper's Key. They set the Cosmic Age, manage the deck, and reset the board. The Key passes clockwise each Age. |
| **Alchemical Formula** | The recipe printed on a Crucible Card, calling for an Attunement, Reagents, and Furnace Heat. Completing one Fires your Stone into the Forge. |
| **Alignment** | The comparison of your Aspects against the Aspects of an Age. Alignment drives both the Harvest and the Siege. |
| **Altar of Kismeta** | Position 8 at the center of the Great Year Board. The first Stone to reach it wins the game. |
| **Ante** | The price of declaring a Siege: Reagents equal in number to the Shield Reagents guarding the target Stone, in any combination of Elements. An Attacker who loses the Siege forfeits it to the supply. |
| **Arcanum** | The tableau zone holding your active Major Arcana cards. |
| **Aspects** | The Sign, ruling Planet, and Element of an Age. |
| **Astral House** | A token built on your Sign, crafted from 3 cards matching that Sign's ruling Planet. It is a Harvest Source, and it may fill one Attunement slot per Firing without being spent. It may be upgraded to an Astral Mansion. |
| **Astral Mansion** | An Astral House upgraded for 2 additional cards of its Sign's ruling Planet, marked with a Reagent token from the supply beneath it. Doubles the House's Alignment score at the Harvest and in a Siege; does not stack with other doubling effects. |
| **Attunement** | A count of Element and Planet slots printed on a Crucible Card. Paid from Spread cards, which are spent, or from Astral Houses, which are not. Required alongside Reagents and Heat to Fire a Formula. |
| **Bazaar** | A shared reference area on the board listing every fixed exchange rate in the game. Formula costs still vary by Crucible Card. |
| **Besieged Bonus** | +2 added to a Defender's total in any later Siege against this Stone for the rest of its current Transmutation Stage. Each successful defense adds another +2. |
| **Burn** | To send a card to the Kismeta discard pile. The loser of a revealed Siege burns every Hand card they committed to it. |
| **Card Lock** | The Spring rule freezing cards in their zones. From Card Lock until Winter, cards cannot move between your Hand and your Spread, though they may still be spent, crafted, traded, or committed. |
| **Coal** | A player-colored disc crafted from 3 cards of one suit, matching a Furnace's Element. Each Coal in a Furnace raises its owner's Heat by +1 in that Element. |
| **Codex** | The reference panel in front of each player on the Great Year Board. Shows the Coal and Reagent costs. |
| **Cosmic Age** | The Sign set by the Agekeeper at the start of each Age, and the Aspects that follow from it. |
| **Crafting** | Spending a set of cards from your Spread to produce Coal or an Astral House. Coal takes 3 cards of one suit; an Astral House takes 3 cards of one Planet. The costs are printed on your Codex and the Bazaar. |
| **Crucible Card** | A red-backed card bearing an Alchemical Formula. Each player holds four. |
| **CV** | The numeric value of a Minor Arcana card, from 1 for an Ace to 14 for a King. |
| **Distill** | To spend 5 Essence of one Element and take the Reagent of that Element. Available on your turn in Summer, Autumn, and Winter. |
| **Duel** | A Summer contest of Zodiac Dice to steal a card from a rival's Spread. |
| **Element** | Fire, Water, Air, or Earth. Every Sign, suit, Furnace, Reagent, and Essence has one. |
| **Essence** | The base currency, paid out at the Harvest in the Element of the Cosmic Age. Five Essence of one Element Distill into one Reagent. |
| **Extinguish** | To remove one Coal from a Furnace. It returns to its owner's supply and their Heat in that Element drops by 1. A Ward can stop it. |
| **Fate Card** | A Major Arcana card that resolves the moment it is drawn. Some carry an effect lasting until Transit. Fates cost nothing, carry no Aspects, and are returned to the discard pile at Transit. |
| **Fire the Stone** | The Autumn action of completing an Alchemical Formula to send your Stone from the Mantle Ring into the Forge, beginning a Transmutation Stage. |
| **Forge** | The space a Stone occupies during a Transmutation Stage. A Stone in the Forge is exposed to Siege for one Summer, guarded by whatever Shield Reagents its owner committed. |
| **Furnace** | One of four shared Elemental spaces, each holding three Coal. You may fuel only the Furnace matching your Sign's Element. |
| **Gift of the Age** | The 1 Essence every player takes in Spring, matching the Cosmic Age's Element. |
| **Great Work** | The central objective: to be the first Alchemist to advance a Philosopher's Stone through all four Transmutation Stages to the Altar of Kismeta. |
| **Great Year** | The full arc of a single game of Kismeta, one grand cosmic cycle. Each round of play is one Cosmic Age within it. |
| **Hand** | The private tableau zone. Hand card effects are inactive and cannot be used in Crafting sets, but Hand cards may be committed in a Siege, where the loser of a revealed Siege burns the cards they committed to it. |
| **Harvest** | The Autumn payout of Essence, scored by comparing each of your Harvest Sources against the Aspects of the Cosmic Age. |
| **Harvest Source** | Anything you own that Aligns with the Cosmic Age to earn Essence at the Harvest. Your Harvest Sources are your Zodiac Die, your Astral Houses, your Adept Cards, and your Spread Cards. |
| **Heat** | Your personal Heat level in an Element, raised +1 by each Coal you own in that Furnace, to a maximum of 3 (Radiant). Formulas call for a level of Heat. |
| **Kismeta Card** | A blue-backed card, either Minor Arcana or Major Arcana. |
| **Major Arcana** | The 22 special cards in the Kismeta Deck: 12 Adepts and 10 Fates. They are held in the Arcanum. |
| **Mantle Ring** | The track your Stone advances along, from START through four Transmutation Stages to the Altar of Kismeta. |
| **Minor Arcana** | The 112 standard cards in the Kismeta Deck, 28 per suit. Each carries a Rank, a CV, a Suit, a Planet, and a printed Effect. |
| **Neutralize** | To cancel the Reversed effect of a card in your Spread. Both cards remain in play. |
| **Perfect Match** | When a single source matches all three Aspects of the Cosmic Age at once (Sign, Planet, and Element) it scores the full stack. |
| **Philosopher's Stone** | Each player's primary game piece. It begins on START and must reach the Altar of Kismeta to win. It may be Resting, Forging, or in Stasis at any given time. |
| **Reagent** | A refined token distilled from 5 Essence of one Element. Reagents pay Formulas, shield a Stone in the Forge, and pay the Ante to declare a Siege. The four types are Sulfur, Vitriol, Quicksilver, and Aqua Regia. |
| **Rekindle** | The Autumn action of paying 5 Essence, of any type, to return a Seized Stone from Stasis to its previous Forge position. |
| **Resonant** | An Adept's enhanced tier, active while the Cosmic Age shares one of its three Aspects. Use its Resonant text in place of Base while this holds. |
| **Reversed** | A penalty that applies only while the card bearing it sits in your Spread. Moving it to your Hand, or Neutralizing it, ends the penalty. |
| **Scry** | To look at the top X cards of the Kismeta Deck, or at a rival's Hand, as the card states. What you learn is yours to keep. |
| **Seized** | The state of a Stone that lost a Siege. It moves to a nearby Stasis zone and must be Rekindled. |
| **Sever** | To switch off one card's printed text until Transit. |
| **Shield Reagent** | A Reagent committed to the Forge beside your Stone as you Fire it, up to 3. Rivals must match their number as an Ante to Siege you, and you take them back when you Temper. |
| **Siege** | A Summer attack on a rival's Stone while it sits in the Forge, resolved by Alignment with a chance for either side to concede before the reveal. |
| **Sign** | One of the twelve Zodiac Signs. Your Sign is set each Spring and holds for the Age. |
| **Spread** | The public tableau zone. Spread card effects are active; they may be used in Crafting sets, and they can be lost in Duels. |
| **Stasis** | Where a Seized Stone is held. From the Autumn of the next Age, its owner may pay 5 Essence to Rekindle it, and it waits there until they can. Marked with a ❄️ snowflake symbol. |
| **Stellium** | A standing effect activated by holding three cards of one Planet in your Spread. It lasts only while all three remain. An Ace is wild for any one Stellium. |
| **Suit** | One of four categories on Minor Arcana cards: Wands, Cups, Pentacles, or Swords. Each corresponds to an Element, a Reagent, and a Furnace. |
| **Tableau** | Everything you have in play: your Spread, Hand, and Arcanum. |
| **Temper** | The Autumn action of advancing a Stone that has spent a full Age in the Forge to the next Mantle Ring position, setting aside its Crucible Card and consuming the Coal it used. |
| **Trade** | A Summer action in which players freely exchange Adept Cards, Astral Houses, Essence, Kismeta Cards, and Reagents. Crucible Cards, Fate Cards, and Coal cannot be traded. |
| **Transit** | The final step of Winter. Duration effects end, Fate Cards are discarded, and the Agekeeper's Key and Cosmic Age Die pass to the next player. |
| **Transmutation Stage** | One step along the Mantle Ring, begun by Firing the Stone and completed by Tempering it. |
| **Ward** | A protection printed on certain cards. The warded card, Stone, or resource cannot be targeted, as the card states. |
| **Zodiac Die** | Your personal 12-sided die. Roll it in Spring to set your Sign for the Age. That Sign holds for Harvests and Sieges alike until the Age ends. |
| **Zodiac Wheel** | The ring of twelve Signs on the Great Year Board, showing each Sign's ruling Planet, Element, and Harvest scoring values. |

---

# LORE: EPILOGUE - The Veil Stirs…

The Philosopher's Stone rests on the Altar of Kismeta, its surface flawless, the ultimate symbol of the Great Work's completion. The voices of the Fates entwine with your thoughts, their whispers as familiar as the rhythms of the cosmos.

Suddenly, the air shifts. A strange, melodic voice enters your mind, disrupting your focus and silencing the Fates.

_"You stand on the outermost shores of known existence, Magnus Alchemista."_

The Philosopher's Stone begins to tremble, its perfect surface fracturing into radiant cracks. It rises from the Altar, shuddering with unstable power, and with a deafening crack, it plunges into the marble, shattering into fragments that scatter across the chamber.

Before you can react, the room dissolves. You're plunged into an endless ocean of shimmering shadow and chaotic light, its currents rippling in fractal patterns that twist and shift in impossible ways. Figures emerge from the Veil, spectral and piscine, their radiant forms floating effortlessly through the shifting expanse, spiraling around you. Their unblinking eyes glow in iridescence.

_"The Veil surrounds you, dear Alchemist. Will you drown in it?_
_No. Breathe it in."_

The currents pull at you, flowing through your thoughts and the core of your being. The weight of the vast, infinite Veil presses against you, revealing glimpses of what lies ahead. A flash of energy ripples through your body, surging upward like a fountain of light, pouring from the crown of your head and back into the oceanic Veil. The spectral figures draw back and watch you intently.

Then, with a final surge, the vision collapses. The Veil dissipates, the figures vanish, and the chamber regains its shape. As your vision sharpens, you notice the fragments of the Philosopher's Stone are gone, as if neither they nor the Veil had ever existed.

The voice returns, faint but resolute:

_"The Veil surrounds you still, dear Alchemist. You must dive deep if you are to ascend."_

---

**KISMETA:**
**The Veiled Ascent**
Coming Soon to the Google Play Store
