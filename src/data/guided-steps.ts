...
const GUIDED_STEPS: GuidedStep[] = [
  ...
  },
  {
    ...
    phaseStep: 3,
    phaseTotal: 5,
    body: "<p>All players begin with 2 Kismeta Cards. You can earn Bonus Cards by aligning items in your inventory with the Cosmic Age's Aspects. Each player calculates their own total first; the Agekeeper then deals the full Harvest.</p><p>Bonus sources: Zodiac Die, Astral Houses, Specialist Cards, and Spread Element Match. Each source scores its single highest-matching Aspect — Sign (+3), Planet (+2), Element (+1).</p>",
    embed: "harvest-order",
    checklist: [
      "Base Harvest: 2 face-down Common Cards per player (always)",
      "Calculate Bonus Cards: each player tallies from all sources (Zodiac Die + Astral Houses + Specialist Cards + Spread Element Match)",
      "Agekeeper's Boon: +2 to every player if Agekeeper's rolled Sign matches the Cosmic Age Sign",
      "Deal the Harvest: Agekeeper deals each player's full total face-down, one at a time",
    ],
    learnMorePath: "play/round-overview",
